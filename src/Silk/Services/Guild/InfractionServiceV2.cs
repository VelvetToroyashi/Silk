...
public class InfractionServiceV2 : IInfractionServiceV2, IHostedService
{
    ...
    private readonly AsyncTimer _dispatchTimer;
    private readonly List<InfractionEntity> _infractionQueue;
    ...
    )
    {
        ...
        _dispatchTimer = new(DispatchInfractionsAsync, TimeSpan.FromSeconds(1));
        
        //Handled by HandleSubmittedInfractionsAsync, which means single-producer/single-consumer.
        _infractionQueue = new();
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _infractionQueue.AddRange(infractions);
        _logger.LogInformation("Loaded {InfractionCount} infractions", _infractionQueue.Count);
        
        
        _dispatchTimer.Start();
        _logger.LogTrace("Started dispatch timer");

        _ = Task.Run(HandleInfractionsAsync);
        _logger.LogTrace("Started infraction handler");
        
        _logger.LogInformation("Started Infraction Service");
    }
    ...
    // We're forced to use Task instead of ValueTask because AsyncTimer sucks.
    private async Task DispatchInfractionsAsync()
    {
        if (!_infractionQueue.Any())
            return;

        foreach (var infraction in _infractionQueue.ToArray())
        {
            if (infraction.ExpiresAt is not { } expiration)
            {
                _logger.LogWarning("Infraction {InfractionId} is queued for dispatch, but does not expire.", infraction.Id);
                _infractionQueue.Remove(infraction);
                continue;
            }
            ...
            InfractionType? type = infraction.Type switch
            {
                InfractionType.Ban  => InfractionType.Unban,
                InfractionType.Mute => InfractionType.Unmute,

                _ => null
            };

            if (type is not { } expectedType)
            {
                _logger.LogWarning("Infraction {InfractionId} is queued for dispatch, but has an unexpected type {InfractionType}.", infraction.Id, infraction.Type);
                continue;
            }
            

            await _submittedInfractions.Writer.WriteAsync((new InfractionRequest(true, "Infraction expired", null, null, infraction.GuildID, expectedType, infraction.TargetID, null), null));
        }
    ...
    private async Task HandleInfractionsAsync()
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (request.Duration.HasValue)
                _infractionQueue.Add(infraction);

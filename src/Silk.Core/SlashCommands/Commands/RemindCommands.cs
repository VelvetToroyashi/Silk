	...
	public sealed class RemindCommands : ApplicationCommandModule
	{
		...
		public sealed class ReminderCommands : ApplicationCommandModule
		{
			...
			private async Task CreateNonRecurringReminderAsync(InteractionContext ctx, string time, string? reminder)
			{
				...
				Optional<TimeSpan> convRes = await conv.ConvertAsync(time);
			...
			public async Task CreateRecurring(
				...
				[Option("occurence", "How often should I remind you?")] ReminderTypeOption type,
				[Option("offset", "How long (from now) should I remind you? Ex: 2h40m, 3d, or `now`.")] string time,
				[Option("reminder", "What do you want to be reminded of?")] string reminder)
			{

...
namespace Silk.Core.Commands.Economy
{
    ...
    public class HighRollerCommand : BaseCommandModule
    {
        private readonly IDbContextFactory<SilkDbContext> _dbFactory;

        public HighRollerCommand(IDbContextFactory<SilkDbContext> dbContextFactory)
        {
        ...
        [Command("baltop")]
        [Aliases("highroller")]
        [Description("See which members have the most money in the Guild!")]
        public async Task BalTop(CommandContext ctx)
        {
            ...
            if (!economyUsers.Any())
            {
                await ctx.RespondAsync($"Seems you don't have any members with economy accounts. Ask some members to use `{ctx.Prefix}daily` and I'll set up an account for them *:)*");
                return;
            }

...
namespace Silk.Core.Types
{
	...
	public abstract class StagedMenu : Menu
	{
		...
		{
			return ctx.Interaction.CreateResponseAsync(InteractionResponseType.UpdateMessage, new DiscordInteractionResponseBuilder().WithContent(ctx.Message.Content).AddMenu(_previous));
		}

		public override Task<bool> CanBeExecuted(ButtonContext args)
		{
			return Task.FromResult(args.User == User);
		}
	}
}
...
namespace Silk.Core.Services
{
    ...
    public sealed class InfractionService : IInfractionService
    {
        ...
        public async Task KickAsync(DiscordMember member, DiscordChannel channel, Infraction infraction, DiscordEmbed embed)
        {
            ...
            GuildConfig config = await _mediator.Send(new GuildConfigGetRequest(member.Guild.Id));
        ...
        public async Task BanAsync(DiscordMember member, DiscordChannel channel, Infraction infraction)
        {
            ...
            GuildConfig config = await _mediator.Send(new GuildConfigGetRequest(member.Guild.Id));
            User user = await _mediator.Send(new UserGetOrCreateRequest(channel.GuildId, member.Id));
            ...
            {
                Guild guild = await _mediator.Send(new GuildGetRequest(member.Guild.Id));
        ...
        public async Task TempBanAsync(DiscordMember member, DiscordChannel channel, Infraction infraction, DiscordEmbed embed)
        {
            ...
            Guild guild = await _mediator.Send(new GuildGetRequest(channel.GuildId));
            GuildConfig config = await _mediator.Send(new GuildConfigGetRequest(channel.GuildId));
            User user = await _mediator.Send(new UserGetOrCreateRequest(guild.Id, member.Id));
        ...
        public async Task MuteAsync(DiscordMember member, DiscordChannel channel, Infraction infraction)
        {
            ...
            User user = await _mediator.Send(new UserGetOrCreateRequest(member.Guild.Id, member.Id));
            Guild guild = await _mediator.Send(new GuildGetRequest(member.Guild.Id));
            if (user.Flags.HasFlag(UserFlag.ActivelyMuted))
            {
                ...
                await _mediator.Send(new GuildUpdateRequest(guild.Id, infraction));
        ...
        public async Task<Infraction> CreateInfractionAsync(DiscordMember member, DiscordMember enforcer, InfractionType type, string reason = "Not given.")
        {
            ...
            _ = await _mediator.Send(new UserGetOrCreateRequest(member.Guild.Id, member.Id));
        ...
        private async Task ApplyInfractionAsync(Guild guild, User user, Infraction infraction)
        {
            ...
            await _mediator.Send(new UserUpdateRequest(guild.Id, user.Id, user.Flags));
        ...
        private async Task OnTick()
        {
            ...
            foreach (var inf in infractions)
            {
                ...
                GuildConfig config = await _mediator.Send(new GuildConfigGetRequest(inf.Key));

    ...
    public class DonateCommand : BaseCommandModule
    {
        ...
        public async Task Donate(CommandContext ctx, uint amount, DiscordMember recipient)
        {
            ...
            if (sender is null && receiver is null)
            {
                await ctx.RespondAsync("Hmm. Seems like neither of you have an account here. " +
                                       $"Go ahead and do `{ctx.Prefix}daily` for me and I'll give you some cash to send to your friend *:)*");
                return;
            }
            ...
            uint amount)
        {
            ...
            await ctx.RespondAsync($"Just verifying you want to send money to this person. " +
                                   $"Could you type `{authKey}` to confirm? (Ignoring this will cancel, since " +
                                   "Velvet can't be bothered to write that logic right now.)");
            
            InteractivityResult<DiscordMessage> message = await interactivity.WaitForMessageAsync(
                m => m.Author == ctx.User && m.Content == authKey.ToString(),
